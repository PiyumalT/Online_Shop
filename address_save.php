<?php
/*if address changed , save address in db , retrive address id
if not , get addressid
save temp order details to order details
remove temp order details
save order table*/
...
if(isset($_REQUEST['place_order'])){
    if(isset($_REQUEST['address_changed'])){
        ...
        $sql="UPDATE temp_orders SET address_id = '$address_id' WHERE order_id = '$temp_order_id'";
        $result = mysqli_query($connect, $sql); 
        if ($result) {
            echo "Insertion success id=38";
        } else {
